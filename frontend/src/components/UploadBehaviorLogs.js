...
import { uploadBehaviorLogs } from "../services/api";
import { motion, AnimatePresence } from "framer-motion";
import { toast } from "sonner";
import { Upload, FileCheck, Loader2, Database, CheckCircle2, RotateCcw } from "lucide-react";

function UploadBehaviorLogs({ onUploadSuccess }) {
    ...
    const handleUpload = async (fileToUpload) => {
        ...
        setIsUploading(true);
        setIsSuccess(false);
        const loadingToast = toast.loading('Ingesting behavioral data...');

        try {
            await uploadBehaviorLogs(fileToUpload);
            toast.success('Heuristic Engine Synced', { 
                id: loadingToast, 
                description: 'Behavioral anomalies have been re-calculated.',
                duration: 4000
            });
            setIsSuccess(true);
    ...
    return (
        ...
        >
            ...
            >
                ...
                <AnimatePresence mode="wait">
                    {isUploading ? (
                        ...
                        >
                            ...
                            <span className="text-sm font-black text-primary uppercase tracking-[0.2em] animate-pulse">Scanning Packets...</span>
                        </motion.div>
                    ) : isSuccess ? (
                        <motion.div 
                            key="success"
                        ...
                        >
                            ...
                            <h4 className="text-lg font-display font-black text-accent-emerald uppercase tracking-tight mb-1">Upload Complete</h4>
                        ...
                        >
                            ...
                            <h4 className="text-lg font-display font-black text-slate-900 dark:text-white uppercase tracking-tight mb-1">Behavior Analytics</h4>
                            <p className="text-xs text-slate-500 dark:text-textMuted font-bold uppercase tracking-wider mb-6 px-4">
                                Drop .CSV to sync behavioral anomalies
                            </p>
                ...
                <div className={`absolute inset-x-0 h-px bg-gradient-to-r from-transparent via-primary/50 to-transparent opacity-0 group-hover:opacity-100 ${isUploading ? 'opacity-100 animate-scan' : ''} pointer-events-none`} />
            </div>
        </motion.div>
    );
}

export default UploadBehaviorLogs;
